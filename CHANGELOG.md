...
- Per-call state isolation: the logger now keys its per-run scratch by `ctx.runHandle` (a per-`accept()` id introduced in `@withboundary/contract@1.5.0`) instead of `contractName`. Concurrent `accept()` calls on the same contract instance — common when a single shared contract serves many parallel requests — each get their own state by construction.

  Bumps the peer dependency to `@withboundary/contract@^1.5.0`.
...
- Tighter wire-event types: `BoundaryLogEvent` is now a discriminated union on `ok` — `AcceptedEvent` (`ok: true, final: true`) and `FailedEvent` (`ok: false`, with `category` and `issues` required). Failure attribution and repair messages can only appear on failed events, enforced at the type level.
  ...
  This is a small breaking change for consumers who treated `BoundaryLogEvent` as a single record with optional everything: code that reads `event.category`, `event.ruleFailures`, etc. now needs to narrow on `event.ok` first. The shape on the wire is unchanged for accepted runs (the previously-optional failure fields are simply never set), so backends that already coalesce on `ok` continue to work without changes.

- Per-attempt logger state is now allocated fresh on every `onAttemptStart` rather than mutated across hooks. A successful terminal event reflects only its own attempt's input, output, and (absence of) failures — there is no cross-attempt scratch slot to forget to clear.

- Added a regression test covering the rule-fail-then-pass path: the terminal event for an accepted run carries no `ruleFailures` / `issues` / `category` / `repairs`.
